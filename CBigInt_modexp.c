...
void bii_modexpodd(bigint *dest, bigint a, bigint e, bigint n){
	...
	bigint r, nprime, tmp1, tmp2, abar, xbar, t, m, u;
	...
	for(int i = bii_sigbits(e)-1; i>=0; i--){
		...
		bigint_multiply(&t, xbar, xbar);
		bigint_multiply(&tmp1, t,nprime);
		bigint_getsigbits(&m,tmp1,rpow);
		bigint_multiply(&xbar,m,n);
		bigint_plusequals(&xbar,t);
		bigint_rightshift(&xbar,rpow);
		if(bigint_cmp(xbar,n)>=0) bigint_minusequals(&xbar,n);
		
		//bii_monproduct(&tmp1, xbar, xbar, n, nprime, rpow);
		//bigint_setval(&xbar, u);
