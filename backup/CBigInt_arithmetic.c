...
void bii_divide(bigint *q, bigint *r, bigint a, bigint b){ //assumes a, b positive
	/*
	 * This is O(log n), where n is the difference between the number of significant bits in a and in b.
	...
	bigint qi;
	bigint_init(&qi);
	bigint_setval(&qi, maxq);
	bigint_rightshift(&qi,1);
	
	bigint diff;
	bigint_init(&diff);
	bigint_setval(&diff, qi);
